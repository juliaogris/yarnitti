...
STRAPS = [(z, i + 1) for i, z in enumerate(HOOP_Z)]  # every skirt ties to the mesh
CLAMPS = [
    200,
    1700,
    3450,
...
def main():
    ...
    out.append(f'<circle cx="{cx}" cy="{cy}" r="{POST_D / 2}" fill="#ddd"/>')
    ...
    out.append(
        f'<text x="{cx}" y="{cy - TUBE_D / 2 - FONT}" text-anchor="middle">from above: sleeve {TUBE_D / 10:.0f} cm across on the {POST_D / 10:.1f} cm post</text>'
    )
    out.append(
    ...
    out.append(
        f'<rect x="{sx - POST_D / 2 * scale:.0f}" y="{sy(POST_H):.0f}" width="{POST_D * scale:.0f}" height="{POST_H * scale:.0f}" fill="#ddd" stroke="#888"/>'
    )
    out.append(
    ...
    core = 0.4195 * STAR_SPAN / 2
    z0, z1 = HOOP_Z[0] + STAR_LIFT, HOOP_Z[0] + STAR_LIFT + 2 * core
    out.append(
    ...
    out.append(
        f'<text x="{sx:.0f}" y="{sy(POST_H) - FONT:.0f}" text-anchor="middle">from the side: sleeve to {TIER_H / 1000:.2f} m in {len(storeys)} storeys, standing on the ground, post to {POST_H / 1000:.1f} m</text>'
    )
    out.append("</g>")
    out.append("</svg>")

    w = sx + 1600
    top = min(-FONT * 3, sy(POST_H) - FONT * 2)
    h = ground + 200 - top
    svg = (
